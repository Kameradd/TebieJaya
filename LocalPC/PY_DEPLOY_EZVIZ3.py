...
TEMP_PROXY_A = 0.05
TEMP_PROXY_B = 20.0
# Periodic logging interval (seconds)
LOG_INTERVAL_SEC = 10
...
def run_pipeline(yolo_weights, video_source, output_csv_path, use_boxmot=True, device="cpu", conf_thres=0.25):
    ...
    goat_data = {}
    last_detected_ids = set()  # retains the last set we logged (for final snapshot)
    ...
    while True: 
        ...
        try:
            ...
            detected_since_last_log = set()  # <-- track only IDs detected in this interval
            ...
            while True:
                ...
                for tid, bbox in parsed:
                    detected_since_last_log.add(tid)  # <-- mark as detected this interval
                ...
                # Periodic append-only logging for detected IDs only
                now = time.time()
                if now - last_log_time >= LOG_INTERVAL_SEC:
                    append_status_snapshot(goat_data, output_csv_path, ids_only=detected_since_last_log, log_type="heartbeat")
                    last_detected_ids = set(detected_since_last_log)  # remember last set we logged
                    detected_since_last_log.clear()
                    last_log_time = now
                ...
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    ...
                    append_status_snapshot(goat_data, output_csv_path, ids_only=detected_since_last_log, log_type="manual_exit")
                    stream.stop(); cv2.destroyAllWindows()
                    return

        except (IOError, cv2.error) as e:
            print(f"❌ Connection Error: {e}\nAttempting to reconnect in 5 seconds...")
            append_status_snapshot(goat_data, output_csv_path, ids_only=detected_since_last_log, log_type="reconnect")
            detected_since_last_log.clear()
            time.sleep(5)
    ...
    # Final snapshot: only the last interval's detected IDs (if any)
    append_status_snapshot(goat_data, output_csv_path, ids_only=last_detected_ids, log_type="final")

# Add helper to append rows without overwriting the CSV
def _append_rows_to_csv(rows, output_csv_path):
    if not rows:
        return
    df = pd.DataFrame(rows)
    os.makedirs(os.path.dirname(output_csv_path), exist_ok=True)
    header = not os.path.exists(output_csv_path) or os.path.getsize(output_csv_path) == 0
    df.to_csv(output_csv_path, mode='a', index=False, header=header, encoding='utf-8')

def append_status_snapshot(goat_data, output_csv_path, ids_only=None, log_type="heartbeat"):
    # Keep original schema; add UTC+8 timestamp at the end
    now_utc = datetime.now(timezone(timedelta(hours=8))).isoformat()
    rows = []
    # Only include IDs provided (i.e., those detected since last interval)
    id_iter = ids_only if ids_only is not None else goat_data.keys()
    for tid in id_iter:
        g = goat_data.get(tid)
        if not g:
            continue
        total_time_seen = g.get("last_seen_time", g["first_seen_time"]) - g["first_seen_time"]
        avg_temp = np.mean([t for (_, t) in g.get("estimated_temp_history", [])]) if g.get("estimated_temp_history") else None
        rows.append({
            "id": tid,
            "event_time_s": "summary",
            ...
            "log_time_utc": now_utc  # UTC+8 timestamp appended last
        })
    _append_rows_to_csv(rows, output_csv_path)

def save_and_exit(goat_data, output_csv_path):
    # Keep function name, switch to append-only behavior
    append_status_snapshot(goat_data, output_csv_path, log_type="final")
    print("✅ Snapshot appended to:", output_csv_path)
